...
{
    float2 uv = i / iResolution.xy;
    uv.x = pow(uv.x, 4.);// adjust curve so everything's not shoved so far to the left

    // sample the FFT texture to generate a volume.
    float samples = 10.;
    float acc = 0.;
    for(float x = 0; x < 1.; x += (1./samples))
    {
       acc += texFFT.Sample(sampFFT, float2(x,0));
    }
    acc /= samples;// now acc is 0-1
    acc = pow(acc, .2);// adjust curve for fun

    // sample a texture, multiply by volume.
    o.r = tex1.Sample(samp1, uv).r * acc;

    // for FFT, x coord = frequency. 0-1. 0 = bass; 1 = treble. lin scale.
    // result has 1 value in red channel, 0-1.
    o.g = texFFT.Sample(sampFFT, float2(uv.x,uv.y)).r;

    float freqAmp = texFFT.Sample(sampFFT, float2(uv.x,0)).r;
    o.b = (uv.y < freqAmp ? 1.0 : 0.0) * acc;
    o.a = 1.;
}

...
function ModConfig:findColorScheme(csName, allowNil)
	...
	return ModColorScheme(self, nil)
end
...
function ModConfig:getColorSchemeName(raw)
	...
	local cs = ModColorScheme(self, raw)
...
function ModColorScheme:__init(config, raw)
	...
	self.raw = raw
	self.config = config
	self.name = config:parseObjectName(raw.Name, "ColorScheme")
	...
	if string.lower(self.name) == string.lower(config:getBaseColorSchemeName()) then
		...
		parent = ModColorScheme(config)
	else
		parent = config:findColorScheme(self.inherits, true)
	end
